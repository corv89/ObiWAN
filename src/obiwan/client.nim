## ObiWAN Gemini Client (Unified)
## 
## This module provides a command-line Gemini protocol client
## that supports both synchronous and asynchronous operation.
## It uses the ObiWAN library and supports configuration via TOML files.
##
## Usage:
##   obiwan-client [options] [<url>]
##
## Options:
...
proc runSync(args: Table[string, Value], config: Config, url: string) =
  ## Run the client in synchronous (blocking) mode
...
proc runAsync(args: Table[string, Value], config: Config, url: string) {.async.} =
  ## Run the client in asynchronous (non-blocking) mode
...
when isMainModule:
  ...
  try:
    ...
    echo "  Mode:         ", if args["--async"]: "Asynchronous" else: "Synchronous"
    ...
    if args["--async"]:
      waitFor runAsync(args, config, url)
    else:
      runSync(args, config, url)
